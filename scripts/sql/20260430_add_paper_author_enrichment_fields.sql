...
-- academic-monitor /api/identity/enrich-paper service.
--
...
COMMENT ON COLUMN papers.author_descriptions IS
    'Per-author profile summary from OpenReview history, indexed by author_order. Populated by academic-monitor /api/identity/enrich-paper.';
COMMENT ON COLUMN papers.author_experiences IS
    'Per-author structured Career & Education history from OpenReview, indexed by author_order. Populated by academic-monitor /api/identity/enrich-paper.';
COMMENT ON COLUMN papers.profile_flags IS
    'Per-author identity flags (is_chinese, is_current_student) + evidence, indexed by author_order. Populated by academic-monitor /api/identity/enrich-paper.';
