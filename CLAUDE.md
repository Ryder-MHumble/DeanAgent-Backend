...
| 消费端 | 服务对象 | 主要接入 API |
|--------|---------|------------|
| Dean-Agent 情报引擎基座服务 | 院长办、院领导 | `/intel/policy` `/intel/personnel` `/intel/daily-briefing` `/intel/tech-frontier` |
...
| Dean-Agent 前端 | (另一个端口) | Next.js (情报引擎基座服务) |
...
- **ScholarDB-System**：学者知识库前端，位于服务器 `/home/ecs-user/Dean-Agent-Project/Dean-Agent-Fronted`（路径名有误导性），使用 Next.js + TypeScript
  ...
  - 使用的 API：`/api/institutions`, `/api/scholars`, `/api/projects`, `/api/events`

- **Dean-Agent**：情报引擎基座服务前端，位于本地 `/Users/sunminghao/Desktop/Dean-Agent`
...
1. **`GET /api/scholars/universities`** ❌ 已删除
   - 原功能：返回学者库中所有高校及其院系列表
   - 替代方案：`GET /api/institutions?view=hierarchy&entity_type=organization`

2. **`GET /api/institutions/scholars/`** ❌ 已删除
   - 原功能：获取所有高校及其院系，包含学者数量统计
   - 替代方案：`GET /api/institutions?view=flat` 或 `?view=hierarchy`

3. **`GET /api/institutions/scholars/stats`** ❌ 已删除
   - 原功能：返回高校、院系、学者的总体统计信息
   - 替代方案：`GET /api/institutions/stats`

4. **`GET /api/institutions/scholars/tree`** ❌ 已删除
   - 原功能：返回按 group → category → institution 三层分类的机构树
   - 替代方案：`GET /api/institutions/taxonomy` 或 `/taxonomy/v2`

**统一使用 `/api/institutions` 接口**：

- **扁平列表视图**：`GET /api/institutions?view=flat&entity_type=organization&region=国内&org_type=高校`
- **层级结构视图**：`GET /api/institutions?view=hierarchy&region=国内&org_type=高校&classification=共建高校`
- **分类体系统计**：`GET /api/institutions/taxonomy` 或 `/taxonomy/v2`（含 sub_classification）
...
```typescript
// 旧代码（已失效）
const response = await fetch(`${API_BASE}/api/scholars/universities?region=国内`);

// 新代码（推荐）
const response = await fetch(`${API_BASE}/api/institutions?view=hierarchy&region=国内&entity_type=organization`);
```
...
**API 返回异常：**
→ `app/api/{articles,sources,dimensions,health}.py`（核心端点）
→ `app/api/intel/{policy,personnel,tech_frontier,university}.py`（业务智能端点）
→ `app/api/sentiment.py`（舆情监控端点：feed/overview/content detail）
...
**舆情 API 数据异常：**
→ `app/api/sentiment.py` + `app/services/external/sentiment_service.py`
...
| projects | id | 项目数据 | 按需 |
| events | id | 学术活动数据 | 63 |
| event_taxonomy | id (UUID) | 活动分类体系（3级） | 14 |
...
**AMiner API 集成**：
```
GET /api/aminer/organizations?name={机构名}     → 查询机构信息（本地 data/institution.json）
GET /api/aminer/scholars/search?name=X&org=Y   → 搜索学者基础信息（AMiner API）
GET /api/aminer/scholars/{aminer_id}           → 获取学者详细信息（AMiner API）

app/api/aminer.py                    → AMiner API 路由（3 个查询端点）
...
**业务智能模块 (intel/)**：
```
app/api/intel/router.py          → intel 子路由（聚合所有业务智能端点）
app/api/intel/policy.py          → 政策智能 API (feed/opportunities/stats)
app/api/intel/personnel.py       → 人事情报 API (feed/changes/stats/enriched-feed/enriched-stats)
app/api/intel/tech_frontier.py   → 科技前沿 API (topics/opportunities/stats/signals)
app/api/intel/university.py      → 高校生态 API (feed/overview/research-outputs)
app/api/intel/daily_briefing.py  → 每日简报 API (today/latest/history)
...
新增业务智能模块时，在 `services/intel/` 下新建子包，在 `api/intel/` 添加端点，在 `intel/router.py` 注册子路由。Pipeline 处理器继承 `pipeline/base.py` 的 HashTracker + save_output_json 共享工具。

**项目库模块 (projects)**：
```
GET  /api/projects/              → 项目列表（分页 + 多维过滤）
GET  /api/projects/stats         → 统计（按状态/类别/资助机构）
GET  /api/projects/{id}          → 项目详情
POST /api/projects/              → 创建项目（ID 自动生成）
PATCH /api/projects/{id}         → 更新项目字段
DELETE /api/projects/{id}        → 删除项目

app/api/projects.py              → Projects API 路由
...
**活动分类体系 (event_taxonomy)**：

三级分类结构（2026-03-18 完成迁移）：
...
```
GET    /api/events/taxonomy           → 获取完整三级分类树
POST   /api/events/taxonomy           → 新增分类节点
PATCH  /api/events/taxonomy/{id}      → 更新分类节点
DELETE /api/events/taxonomy/{id}      → 删除分类节点（级联）

GET    /api/events/?category=科研学术&series=XAI智汇讲坛  → 按分类筛选活动
```

配置与脚本：

```
config/event_taxonomy.yaml                    → 分类体系配置（初始化用）
（历史）分类初始化/迁移脚本已清理，不再作为仓库维护入口
app/services/core/event_taxonomy_service.py   → 分类体系服务层
```

数据迁移状态：

- ✅ 63 个活动已迁移到「科研学术 - XAI智汇讲坛」分类
- ✅ event_taxonomy 表已初始化（3个L1 + 8个L2 + 3个L3）
- ✅ 前端可通过 /taxonomy 端点获取动态分类树
...
## Institutions 表字段清理（2026-03-18）

**清理了 14 个无用字段**，所有字段均为空或只包含空数组：

已删除字段：

- `committees`, `labs`, `cooperation_info` - 完全为空
- `sources` - 43/187 有数据但都是空数组（原用于爬虫源识别，已废弃）
- `key_departments`, `joint_labs`, `training_cooperation`, `academic_cooperation` - 完全为空
- `talent_dual_appointment`, `recruitment_events`, `visit_exchanges`, `cooperation_focus` - 完全为空
- `custom_fields` - 完全为空
- `mentors` - 11/187 有数据但都是空数组

SQL 清理脚本：`scripts/sql/cleanup_institutions_fields.sql`

已更新的代码文件：

- `app/schemas/institution.py` - 移除字段定义和 `DepartmentSource`/`MentorInfo` 类
- `app/services/core/institution_service.py` - 移除字段引用
- `app/services/core/institution/detail_builder.py` - 移除字段构建逻辑
...
1. **GET /api/institutions/taxonomy** - 获取分类体系（用于渲染导航栏）
...
2. **GET /api/institutions?region=国内&org_type=高校&classification=共建高校** - 根据筛选条件获取机构列表
...
```bash
# 测试分类体系API
curl "http://10.1.132.21:8001/api/institutions/taxonomy"

# 测试筛选API（注意：中文需要URL编码）
curl "http://10.1.132.21:8001/api/institutions?region=%E5%9B%BD%E5%86%85&org_type=%E9%AB%98%E6%A0%A1&page_size=5"
```
...
## Scripts 通用框架（2026-03-18 新增）

**目标**：消除 scripts/ 目录下 33 个脚本中约 28% 的重复代码（~2000 行），提高开发效率。

### 核心组件（scripts/core/）

所有通用组件都在 `scripts/core/` 目录下，写新脚本时优先使用这些组件：

| 组件 | 文件 | 功能 | 减少重复 |
|------|------|------|---------|
| BaseScript | base_script.py | 脚本基类（日志/参数/数据库/错误处理） | ~100 行/脚本 |
| DatabaseScript | base_script.py | 数据库操作基类（CRUD/批量操作） | ~80 行/脚本 |
| DataProcessingScript | base_script.py | 数据处理基类（统计追踪） | ~60 行/脚本 |
| ProgressTracker | progress_tracker.py | 进度追踪（断点续传/失败记录） | ~50 行/脚本 |
| AsyncAPIClient | api_client.py | 异步 HTTP 客户端（速率限制/缓存） | ~40 行/脚本 |
| AMinerClient | api_client.py | AMiner API 专用客户端 | ~60 行/脚本 |
| DataTransformer | data_transformer.py | 数据转换（字段映射/解析/验证） | ~60 行/脚本 |
| FileUtils | file_utils.py | 文件 I/O（JSON/CSV/YAML） | ~30 行/脚本 |
...
class MyDatabaseScript(DatabaseScript):
    def __init__(self):
        super().__init__(
            name="my_script",
            description="脚本描述",
    ...
    async def run_async(self):
        ...
        await self.update_batch(records, id_field="id")
        await self.delete_batch(ids)

if __name__ == "__main__":
    MyDatabaseScript().execute()
```

**自动支持参数**：`--dry-run`, `--limit`, `--resume`, `--force`, `--verbose`
...
class MyDataScript(DataProcessingScript):
    def __init__(self):
        super().__init__(name="my_script", description="描述")

    async def run_async(self):
        await self.init_database()

        # 加载数据
        data = FileUtils.load_json("input.json")
        ...
        for item in data:
            ...
            # 验证必填字段
            is_valid, missing = DataTransformer.validate_required_fields(
                record, ["name"]
            )
        ...
        self.print_stats()  # 打印统计

if __name__ == "__main__":
    MyDataScript().execute()
```

**3. API 调用脚本**（AMiner/LLM 富化）

```python
from scripts.core import BaseScript, AMinerClient, ProgressTracker, FileUtils

class MyAPIScript(BaseScript):
    def __init__(self):
        super().__init__(name="my_script", description="描述")
        self.tracker = ProgressTracker("my_task")
    ...
    async def run_async(self):
        # 初始化 API 客户端
        client = AMinerClient(self.args.api_key)

        # 加载数据
        scholars = FileUtils.load_csv("input.csv")
        ...
        for i, scholar in enumerate(scholars[start_index:], start=start_index):
            # API 调用（自动速率限制）
            results = await client.search_scholars(scholar["name"])

            if results:
                detail = await client.get_scholar_detail(results[0]["id"])
                enriched.append(detail)

            # 保存进度（每 10 条）
            if i % 10 == 0:
                self.tracker.save_progress(i)

        # 保存结果
        FileUtils.save_json("output.json", enriched)
...
```python
from scripts.core import DataTransformer

# 解析机构名称（中英文分离）
inst = DataTransformer.parse_institution_name("清华大学 Tsinghua University")
# 返回: {"zh": "清华大学", "en": "Tsinghua University"}

# 解析 JSON 字段
tags = DataTransformer.parse_json_field('["AI", "ML"]')  # 返回列表

# 字段映射（带类型转换）
mapping = {"name": "姓名", "age": ("年龄", "int")}
result = DataTransformer.map_fields(source, mapping)
...
```python
from scripts.core import FileUtils
...
# CSV
records = FileUtils.load_csv("data.csv")
FileUtils.save_csv("output.csv", records)

# YAML
config = FileUtils.load_yaml("config.yaml")
```

**ProgressTracker（进度追踪）**

```python
from scripts.core import ProgressTracker

tracker = ProgressTracker("task_name")

# 断点续传
if args.resume and tracker.is_resumable():
    start_index = tracker.get_last_index()

# 保存进度
tracker.save_progress(last_index, processed_ids)

# 保存失败记录
tracker.append_failed({"item": item, "error": str(e)})
```

### 代码减少对比

| 脚本类型 | 原始行数 | 使用框架后 | 减少比例 |
|---------|---------|-----------|---------|
| 数据库清理 | 80+ | ~30 | 62% |
| 数据导入 | 80+ | ~40 | 50% |
| AMiner 富化 | 572 | ~150 | 73% |
| 数据处理 | 363 | ~100 | 72% |

### 最佳实践

1. **选择合适的基类**
   - 数据库操作（清理/迁移/验证）→ `DatabaseScript`
   - 数据处理/转换/导入 → `DataProcessingScript`
   - API 调用/复杂逻辑 → `BaseScript`

2. **必须使用的功能**
   - 长时间任务 → `ProgressTracker`（支持断点续传）
   - 写操作 → 支持 `--dry-run`（框架自动提供）
   - API 调用 → `AsyncAPIClient` 或 `AMinerClient`（自动速率限制）

3. **统一日志**
   - 使用 `self.logger` 而不是 `print()`
   - 日志自动保存到 `logs/scripts/{name}.log`

4. **字段映射**
   - 使用 `DataTransformer.map_fields()` 而不是手写循环
   - 支持类型转换：`("字段名", "int|float|bool|list")`

### 完整示例

`scripts/examples/` 目前保留：
- `aminer_detail_api_diagnostic.py` - AMiner 学者详情 API 诊断示例

### 迁移现有脚本

1. 识别脚本类型（数据库/数据处理/API）
2. 选择基类（DatabaseScript/DataProcessingScript/BaseScript）
3. 将初始化代码移到 `__init__()`
4. 将参数解析移到 `add_arguments()`
5. 将主逻辑移到 `run_async()`
6. 替换重复代码为框架方法
7. 测试 `--dry-run` 和 `--resume` 功能
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/reports/generate` | POST | 生成指定维度的分析报告 |
| `/api/reports/sentiment/latest` | GET | 快捷获取最近N天舆情报告 |
| `/api/reports/dimensions` | GET | 列出所有支持的报告维度 |

### 使用示例

```bash
# 生成最近 7 天的舆情报告
curl "http://10.1.132.21:8001/api/reports/sentiment/latest?days=7"

# 生成指定日期范围的报告
curl -X POST "http://10.1.132.21:8001/api/reports/generate" \
...
3. 在 `app/api/reports.py` 添加新维度的处理逻辑
