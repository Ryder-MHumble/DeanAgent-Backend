...
```
GET    /api/v1/events/taxonomy           → 获取完整三级分类树
POST   /api/v1/events/taxonomy           → 新增分类节点
PATCH  /api/v1/events/taxonomy/{id}      → 更新分类节点
DELETE /api/v1/events/taxonomy/{id}      → 删除分类节点（级联）

GET    /api/v1/events/?category=科研学术&series=XAI智汇讲坛  → 按分类筛选活动
```
...
scripts/migration/init_event_taxonomy.py      → 从 YAML 初始化数据库
scripts/migration/migrate_event_categories.py → 迁移现有活动分类
...
参考 `scripts/examples/` 目录：
- `database_cleanup_example.py` - 数据库清理（30 行，原 80 行）
- `data_import_example.py` - 数据导入（40 行，原 80 行）
- `aminer_enrichment_example.py` - AMiner 富化（150 行，原 572 行）
